...
class ListLayout extends ElementorWidget {
	...
	protected function render() {
		...
		if ( $base_arg['source'] === 'product' ) {
			...
			if ( ! empty( $args['tax_query'] ) && is_array( $args['tax_query'] ) ) {
				array_walk_recursive(
					$args['tax_query'],
					function ( &$value, $key ) {
						if ( 'taxonomy' === $key && 'food-menu-cat' === $value ) {
							$value = 'product_cat';
						}
					}
				);
		...
		if ( '4' == $base_arg['dCols'] ) {
			$desktop_col = 3;
		} elseif ( '3' == $base_arg['dCols'] ) {
			$desktop_col = 4;
		} elseif ( '2' == $base_arg['dCols'] ) {
			$desktop_col = 6;
		} else {
			$desktop_col = '2';
		}
		echo "<div data-layout='" . esc_attr( $layout ) . "' data-desktop-col='" . esc_attr( $desktop_col ) . "' class='fmp-" . esc_attr( $layout ) . ' ' . esc_attr( $animation ) . " fmp-container-fluid fmp-wrapper fmp-list-layout fmp-elementor-widget fmp-list-layout-wrapper ' id='fmp-container-" . esc_attr( $layout ) . " '
		>";

		echo "<div
		
		data-el-query='" . esc_attr( $query_json ) . "'
		data-el-settings='" . esc_attr( $settings_json ) . "'>";

		echo "<div class='fmp-row " . esc_attr( $masonry ) . ' ' . esc_attr( $ajax_class ) . ' fmp-elementor-layout fmp-content-loader fmp-' . esc_attr( $layout ) . " fmp-read-more-active'>";
		...
		if ( $base_arg['posts_loading_type'] === 'ajax-number-pagination' ) {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Read-only pagination number for display; cast to int.
		...
		if ( $base_arg['posts_loading_type'] === 'ajax-load-more-button' ) {
			echo '<div class="fmp-pagination"><button class="fmp-load-more-btn">' . esc_html__( 'Load More', 'tlp-food-menu' ) . '</button></div>';
		}
		echo '</div>';
		echo '</div>';
		?>
